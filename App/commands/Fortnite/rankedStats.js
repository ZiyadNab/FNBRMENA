    ...
    callback: async (FNBRMENA, message, args, text, Discord, client, admin, userData, alias, emojisObject) => {
        ...
        const drawRankedStats = async (data, msg) => {
            ...
            try {
                ...
                else if (data.rankedData.rankingType === "ranked_blastberry_build" || data.rankedData.rankingType === "ranked_blastberry_nobuild") {
                    if(userData.lang === "ar")  var rankingType = await Canvas.loadImage(`https://cdn2.unrealengine.com/ar-fnbr-s30-potc-logo-350x300-350x300-028a5c289d47.png`)
                    else  var rankingType = await Canvas.loadImage(`https://cdn2.unrealengine.com/en-fnbr-s30-potc-logo-350x300-350x300-3ce46cdfc3ba.png`)
                }
                ...
                FNBRMENA.RankedStats(text, userInput.platform, userInput.playlist)
                    .then(res => {
                        ...
                        drawRankedStats({
                            ...
                            rankedData: res.data.rankedData.find(e => e.trackguid === collected.values[0])
                        }, dropMenuMessage)
            ...
            if (collected.customId === `current-${alias}`) {
                ...
                FNBRMENA.RankedStats(text, userInput.platform, userInput.playlist)
                    .then(res => {
                        ...
                        drawRankedStats({
                            account: res.data.account,
                            rankedData: res.data.rankedData[res.data.rankedData.length - 1]
                        }, dropMenuMessage)
