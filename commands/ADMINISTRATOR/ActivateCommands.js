        ...
        admin.database().ref("ERA's").child("Commands").child(args[0]).once('value', async data => {
            if(data.exists()){
                if(args[1] === "true" || args[1] === "false"){
                    ...
                    .setColor('#BB00EE')
                    .setTitle(":x: Make sure that you enter TRUE to enable the command or FALSE to disable it etherwise it will throw an error")
                    message.channel.send(err)
                }
            }else{
                const error = new Discord.MessageEmbed()
                    .setColor('#BB00EE')
                    .setTitle(":x: Make sure that you have entered a valid command")
