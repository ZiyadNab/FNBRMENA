...
module.exports = {
    ...
    callback: async (FNBRMENA, message, args, text, Discord, client, admin, userData, alias, emojisObject) => {
        ...
        //pak viewer
        const pak = async (pak, msg) => {

            // Request data
            await FNBRMENA.PAK(userData.lang, `dynamicPakId=${pak}`)
            .then(async res => {
                ...
                if(err.response.data.status === 404){
                    ...
                    msg.edit({embeds: [noPakHasBeenFoundError], components: [], files: []})
                    .catch(err => {
                        FNBRMENA.Logs(admin, client, Discord, message, alias, userData.lang, text, err, emojisObject, msg)
                    })

                }else FNBRMENA.Logs(admin, client, Discord, message, alias, userData.lang, text, err, emojisObject, msg) // Other errors
            })
        }
        ...
        .then(async res => {
            ...
            for(const item of res.data.data){
                if(!foundPaks.includes(item.dynamicPakId)){
                    ...
                    avalabilePaks.push({
                        ...
                        emoji: `${emojisObject[item.rarity.value].name}:${emojisObject[item.rarity.value].id}`
                    })
                }
            }
            ...
            .then(async collected => {
                ...
                if(collected.customId === "Paks") pak(collected.values[0], dropMenuMessage)
