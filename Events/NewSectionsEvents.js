                ...
                .then(async res => {
                    ...
                    if(number === 0){
                        ...
                        for(let i = 0; i < res.data.shopSections.sectionList.sections.length; i++){
                            response[counter++] = await res.data.shopSections.sectionList.sections[i]
                        }
                        counter = 0
                        number++
                    }
                    ...
                    for(let i = 0; i < res.data.shopSections.sectionList.sections.length; i++){
                        sections[counter++] = await res.data.shopSections.sectionList.sections[i]
                    }
                    ...
                    if(JSON.stringify(sections) !== JSON.stringify(response)){
                        ...
                        for(let i = 0; i < sections.length; i++){
                            ...
                            if(!response.includes(sections[i])) added[counter++] = await sections[i]
                        }
                        ...
                        for(let i = 0; i < response.length; i++){
                            ...
                            if(!sections.includes(response[i])) removed[counter++] = await response[i]
                        }
                        ...
                        //store data
                        for(let i = 0; i < res.data.shopSections.sectionList.sections.length; i++){
                            response[i] = await res.data.shopSections.sectionList.sections[i]
                        }
