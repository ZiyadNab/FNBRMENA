...
module.exports = async (message, args, service, Discord, client, admin, errorEmoji, checkEmoji, loadingEmoji) => {
    ...
    if(activeFortniteXMPP.includes(message.author.id)){
        ...
        if(databaseProfile.data().status){
            ...
            //try to clear the emote if there is an error catch it
            try {

                //clear emote
                await service.party.members.get(databaseProfile.data().credintials.accountId).clearEmote()

                //emote has been cleared
                const emoteCleared = new Discord.MessageEmbed()
                emoteCleared.setColor(FNBRMENA.Colors("embed"))
                if(lang === "en") emoteCleared.setTitle(`The emote has been cleared successfully. ${errorEmoji}`)
                else if(lang === "ar") emoteCleared.setTitle(`تم ايقاف الرقصة بنجاح ${errorEmoji}`)
                message.reply(emoteCleared)

            } catch {

                //if there is an error while clearing the emote
                const clearEmoteError = new Discord.MessageEmbed()
                clearEmoteError.setColor(FNBRMENA.Colors("embed"))
                if(lang === "en") clearEmoteError.setTitle(`There was an error while clearing the Emote, please try again later. ${errorEmoji}`)
                else if(lang === "ar") clearEmoteError.setTitle(`يوجد مشكلة اثناء ايقاف الرقصة. يرجى المحاولة لاحقا ${errorEmoji}`)
                message.reply(clearEmoteError)
            }
        }else{
