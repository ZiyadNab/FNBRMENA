...
module.exports = {
    ...
    callback: async (FNBRMENA, message, args, text, Discord, client, admin, userData, alias, emojisObject) => {
        ...
        const values = []
        const colllector = await message.channel.createMessageComponentCollector({filter, time: 2 * 60000, errors: ['time'] })
        colllector.on('collect', async collected => {

            //if cancel button has been clicked
            if(collected.customId === "Cancel") colllector.stop()

            //if the user chose a platform
            if(collected.customId === "platform"){
                collected.deferUpdate()
                values.push(collected.values[0])
            }

            //if the user clicked on a button
            if(collected.customId === "Generate" || collected.customId === "Custom"){

                //check if user chose a platform
                if(values.length !== 0){

                    var customGBColors = null
                    var customGBOutfit = null
                    var customGBLoadingscreen = null

                    if(collected.customId === "Custom"){

                        // Create the modal and add text fields
                        const customModal = new Discord.ModalBuilder()
                        customModal.setCustomId('customModal')
                        if(userData.lang === "en"){
                            customModal.setTitle('Stats Customization') //set modal title
                            customModal.addComponents( // add fields
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('color1')
                                    .setLabel("Enter color one (Type * to skip)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('color2')
                                    .setLabel("Enter color two (Type * to skip)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('outfitID')
                                    .setLabel("Enter outfir id (Type * to skip)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('loadingscreenID')
                                    .setLabel("Enter loadin gscreen id (Type * to skip)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                )
                            )
                        }else if(userData.lang === "ar"){
                            customModal.setTitle('تخصيص الأحصائيات') //set modal title
                            customModal.addComponents( // add fields
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('color1')
                                    .setLabel("ادخل اللون الأول (اكتب * للتحطي)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('color2')
                                    .setLabel("ادخل اللون الثاني (اكتب * للتحطي)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('outfitID')
                                    .setLabel("ادخل معرف الزي (اكتب * للتحطي)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                ),
                                new Discord.ActionRowBuilder().addComponents(
                                    new Discord.TextInputBuilder()
                                    .setCustomId('loadingscreenID')
                                    .setLabel("ادخل معرف شاشة التحميل (اكتب * للتحطي)")
                                    .setStyle(Discord.TextInputStyle.Short)
                                )
                            )
                        }

                        //show model to the user
                        await collected.showModal(customModal)

                        //listen for modal submission
                        const modalFilter = (interaction) => interaction.customId === 'customModal';
                        await collected.awaitModalSubmit({modalFilter, time: 2 * 60000})
                        .then(async modalCollect => {
                            modalCollect.deferUpdate();

                            customGBColors = `${modalCollect.fields.getTextInputValue('color1')},${modalCollect.fields.getTextInputValue('color2')}`
                            customGBOutfit = await modalCollect.fields.getTextInputValue('outfitID')
                            customGBLoadingscreen = await modalCollect.fields.getTextInputValue('loadingscreenID')
                            
                        })

                    }else if(collected.customId === "Generate") collected.deferUpdate()

                    //stop the collcetor
                    colllector.stop()

                    //request data
                    await FNBRMENA.Stats(text, values[0], "lifetime")
                    .then(async res =>{
                        ...
                        .then(async msg => {
                            ...
                            const randomOutfit = async (random, id) => {

                                //if there is no custom id provided
                                if(random){
                                    ...
                                    //request data
                                    await FNBRMENA.Search(userData.lang, "custom", `&id=${id}`)
                                    .then(async listOfOutfits => {

                                        //outfit img 
                                        if(listOfOutfits.data.items[0].images.featured) var outfitIMG = await Canvas.loadImage(listOfOutfits.data.items[0].images.featured)
                                        else var outfitIMG = await Canvas.loadImage(listOfOutfits.data.items[0].images.icon)
                                        ctx.drawImage(outfitIMG, canvas.width - 1250, 300, 1860, 1860)
                                    })
                                }
                            }
                            ...
                            //get a random color
                            if(customGBColors){
                                if(!customGBColors.includes("*")) var randomColor = customGBColors
                                else var randomColor = listOfColors[await randomNumber(listOfColors.length)]
                            }else var randomColor = listOfColors[await randomNumber(listOfColors.length)]
                            ...
                            if(customGBOutfit){
                                if(!customGBOutfit.includes("*")) await randomOutfit(false, customGBOutfit)
                                else await randomOutfit(true, null)
                            }else await randomOutfit(true, null)
                        ...
                        if(err.response.data.status === 404){
                            ...
                            if(err.response.data.error === "the requested account does not exist"){
                                ...
                                //epic games string
                                if(platforms[num] === "epic" && userData.lang === "en") var usedPlatform = 'Epicgames'
                                else if(platforms[num] === "epic" && userData.lang === "ar") var usedPlatform = 'ايبك قيمز'

                                //psn string
                                if(platforms[num] === "psn" && userData.lang === "en") var usedPlatform = 'Playstation'
                                else if(platforms[num] === "psn" && userData.lang === "ar") var usedPlatform = 'بلايستيشن'

                                //xbl string
                                if(platforms[num] === "xbl" && userData.lang === "en") var usedPlatform = 'XBOX'
                                else if(platforms[num] === "xbl" && userData.lang === "ar") var usedPlatform = 'اكسبوكس'
                    ...
                    collected.reply({content: 'Please specifiy a platform first', ephemeral: true})
                }
            }
        })
