...
module.exports = (FNBRMENA, client, admin, emojisObject) => {
                    ...
                    if(JSON.stringify(res.data.lastUpdate.uid) !== JSON.stringify(UID)){
                        ...
                        snapshot.forEach(async doc => {
                            ...
                            for(let j = 0; j < res.data.shop.length; j++){
                                const item = await docRef.doc(`${doc.id}`).collection("Reminders").doc(res.data.shop[j].mainId).get()

                                if(item.exists){
                                    //the item has been released

                                    FNBRMENA.searchByID(item.data().lang, res.data.shop[j].mainId)
                                    .then(async userITEM => {
                                        ...
                                        const applyText = (canvas, text, width, font) => {
                                            ...
                                            let fontSize = font;
                                            do {
                                                if(item.data().lang === "en") ctx.font = `${fontSize -= 1}px Burbank Big Condensed`;
                                                else if(item.data().lang === "ar") ctx.font = `${fontSize -= 1}px Arabic`;
                                        ...
                                        if(userITEM.data.item.introduction != null){
                                            ...
                                            if(item.data().lang === "en") var seasonChapter = `C${chapter}S${season}`
                                            else if(item.data().lang == "ar")var seasonChapter = `الفصل ${chapter} الموسم ${season}`

                                        }else{

                                            if(item.data().lang === "en") var seasonChapter = `${userITEM.data.item.added.version}v`
                                            else if(item.data().lang == "ar")var seasonChapter = `تحديث ${userITEM.data.item.added.version}`
                                            ...
                                            for(let j = 0; j < userITEM.data.item.gameplayTags.length; j++){
                                                if(userITEM.data.item.gameplayTags[j].includes('Source')){
                                                    ...
                                                    if(userITEM.data.item.gameplayTags[j].toLowerCase().includes("itemshop")){
                                                        ...
                                                        if(item.data().lang === "en") var Source = "ITEMSHOP"
                                                        else if(item.data().lang === "ar") var Source = "متجر العناصر"
                                                    }else if(userITEM.data.item.gameplayTags[j].toLowerCase().includes("battlepass")){

                                                        if(item.data().lang === "en") var Source = "BATTLEPASS"
                                                        else if(item.data().lang === "ar") var Source = "بطاقة المعركة"
                                                    }else if(userITEM.data.item.gameplayTags[j].toLowerCase().includes("event")){

                                                        if(item.data().lang === "en") var Source = "EVENT"
                                                        else if(item.data().lang === "ar") var Source = "حدث"
                                                        ...
                                                        if(item.data().lang === "en") var Source = "EXCLUSIVE"
                                                        else if(item.data().lang === "ar") var Source = "حصري"
                                                    }
                                        ...
                                        //setting up moment
                                        moment.locale(item.data().lang)
                                        const Now = moment()
                                        const long = moment(item.data().date)
    ...
    setInterval(remindersEvent, 2 * 60000)
}